...
import { getUserByName, addPokemon } from '../controllers/pokemon.controller';
...
router.post('/add', (req, res, next) => {
  ...
  addPokemon(pokemon, (err, pokemon) => {
    if (err) {
      ...
      res.json({
        success: false,
        msg: 'Unable to add Pokemon, please try agian later!'
      });
    } else {
...
router.get('/getbyname', (req, res, next) => {
  ...
  getUserByName(name, (err, res) => {
    if (err) throw err;
    else res.json({ success: true, msg: name });
  });
});
